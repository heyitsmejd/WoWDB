...
router.get('/', function(req,res){
        ...
        db.connection.query("SELECT * FROM streams ORDER BY id DESC", function(error, results, fields) {
...
router.post('/get', function(req,res){
  console.log(req.body.id)
        db.connection.query("SELECT * FROM streams WHERE id = ?", req.body.id, function(error, results, fields) {
            //console.log(user.user + " fetched results")
            res.json(results);
          });
...
router.post('/add', function(req, res) {
     ...
     { 
        ...
        if (error) return res.status(403).json({type: 'error', message: 'Provided token is invalid.', error})
            let deleted = 0;
            let visible = 1;
            db.connection.query("INSERT INTO streams (`userId`, `name`, `channel`, `tags`, `platform`, `class`) VALUES ('" + req.body.userId + "','" + req.body.name + "','" + req.body.channel + "','" + req.body.tags + "','" + req.body.platform + "','" + req.body.class + "')", function(error, results, fields) {
              //if(error) console.log(error);
